#ifndef EXAMPLE_UTIL_GIA_UTIL_H
#define EXAMPLE_UTIL_GIA_UTIL_H
...
public:
    ...
    explicit gia_sequence(const std::string& seq_dir_path, const int& start_id);

    virtual ~gia_sequence() = default;

    std::vector<frame> get_frames() const;

private:
    std::vector<double> real_timestamps_;
    std::vector<double> recorded_timestamps_;
    std::vector<std::string> left_img_file_paths_;
    std::vector<std::string> right_img_file_paths_;
};

#endif // EXAMPLE_UTIL_GIA_UTIL_H
